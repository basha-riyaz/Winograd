...
import { ChevronDown, Server, FileCode, Trash2 as TrashIcon, Bold } from "lucide-react"
  ...
  <Dialog open={isModalOpen} onOpenChange={setModalOpen}>
    <DialogContent className="sm:max-w-[425px]">
      <DialogHeader>
        ...
        <DialogDescription>
          Are you sure you want to delete <span className="font-bold text-red-700">{agentToDelete?.name}</span> agent? This action cannot be undone.
        </DialogDescription>
      </DialogHeader>
      <DialogFooter>
        <Button 
        ...
        <Button 
          variant="destructive"
          onClick={async () => {
            if (agentToDelete) await handleDeleteAgent(agentToDelete.id);
            setModalOpen(false);
          }}
        >
          Confirm Delete
              ...
              <DropdownMenuContent align="end" className="w-[220px]">
                ...
                {savedAgents.length > 0 ? (
                  savedAgents.map((agent) => (
                    ...
                    >
                      <TrashIcon className="h-4 w-4 text-primary/70" />
                      {agent.name}
                    </DropdownMenuItem>
