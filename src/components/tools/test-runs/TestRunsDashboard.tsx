...
export function TestRunsDashboard() {
  ...
  useEffect(() => {
    // If there's an id in the query params, auto-select that run
    if (runs && runs.length > 0 && searchParams && searchParams.get('id')) {
      const found = runs.find(r => r.id === searchParams.get('id'));
      if (found) setSelectedRun(found);
    }
  }, [runs, searchParams]); // This effect runs every time 'runs' or 'searchParams' changes
