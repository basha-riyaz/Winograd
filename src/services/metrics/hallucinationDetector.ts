...
const HALLUCINATION_PROMPT_TEMPLATE = `
You are a strict and logical evaluator. Your only task is to determine if an AI's response meets the required criteria for a given test.
...
PASSING CRITERIA (Expected Output):
{expectedOutput}
...
[INSTRUCTIONS]
Carefully compare the "AI RESPONSE TO EVALUATE" against the "PASSING CRITERIA". The test passes if the AI response contains all the key information and concepts described in the criteria.

Does the AI response meet the passing criteria?

Respond in this exact JSON format:
{
  "isHallucination": boolean,
  "reasoning": "Briefly explain why the response passed or failed to meet the criteria."
}

- Set "isHallucination" to false if the response MEETS the criteria.
- Set "isHallucination" to true if the response FAILS to meet the criteria.
...
export class HallucinationDetector {
  ...
  async detectHallucination(
    ...
    const filledPrompt = HALLUCINATION_PROMPT_TEMPLATE
      // .replace("{agentDescription}", this.agentDescription)
      .replace("{query}", query)
      .replace("{expectedOutput}", context)
      .replace("{response}", response);

      // console.log('agentDescription========(tum): ', this.agentDescription);
      console.log('query========: ', query);
      console.log('context========: ', context); //user input and ai response  (context should get expected output)
