...
// console.log(`"scenario : "  ${scenario} "expectedOutput : "  ${expectedOutput}`)
  ...
  private async processTurn(message: string, chatId: string, expectedOutput: string): Promise<{
    response: string;
    responseTime: number;
    ...
    try {
      ...
      console.log('apiResponse:==============(ryz) ', apiResponse);
    ...
    console.log('assistantResponse:==============(ryz) ', typeof assistantResponse);

    // Extract structured JSON from assistantResponse
    const structuredJson = await this.extractStructuredJsonFromResponse(assistantResponse);
    ...
    await dbService.saveConversationMessage({
      ...
      conversationId: chatId,
      role: 'assistant',
      content: JSON.stringify(structuredJson, null, 2),
      timestamp: new Date().toISOString(),
      metrics: messages[1].metrics // Use the metrics object that includes hallucination detection result
    });

    return { response: structuredJson, responseTime, messages };
  }


  //riyaz made this change
  /**
   * Extract structured JSON from assistant response using a prompt
   */
  public async extractStructuredJsonFromResponse(assistantResponse: string): Promise<any> {
    const extractionPrompt = `

    You are a highly precise data extraction engine. Your task is to convert unstructured text into a structured, concise JSON object. You must follow the instructions exactly.

**Instructions:**

1.  **Analyze the text** to identify distinct headings (e.g., "Experience Level:", "Key Skills:").
2.  **For EACH heading, create a short, descriptive, "snake_case" JSON key** (e.g., "Experience Level" becomes "experience_level").
3.  **Extract the information** that follows EACH heading and use it as the value.
4.  If a value is a list of bullet points, **combine the items into a single, comma-separated string**.
5.  **CRITICAL: DO NOT merge information from different headings into a single key.** Each heading must have its own unique key-value pair in the final JSON.
6.  **CRITICAL (No Redundancy):** The value for each key must be unique and concise. **DO NOT repeat information or phrases.** If two bullet points say the same thing, only include the information once.

**Here is an example of the required transformation:**
...
`;
    ...
    try {
      ...
      console.error('Error extracting structured JSON:', error);
      throw error;
    }
  }
  ...
  async runTest(scenario: string, expectedOutput: string): Promise<TestResult> {
    ...
    try {
      ...
      const initialMessage = await this.generateInitialMessage(scenario, expectedOutput);
      console.log('initialMessage===========: ', initialMessage);
      const firstTurn = await this.processTurn(initialMessage, chatId, expectedOutput);
      // const structuredJson = await this.extractStructuredJsonFromResponse(firstTurn.response);
      ...
      const validatedMessages = allMessages;
      console.log('allMessages: ------------ :', validatedMessages);
