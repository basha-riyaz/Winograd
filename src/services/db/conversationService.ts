...
const data = [  
        {
          agent: "Bench Bridgeeee",
          testRuns: [
            { date: "2025-12-20", health: "High", hallucinationRate: 10 },
            { date: "2025-12-23", health: "Moderate", hallucinationRate: 10 },
            { date: "2025-12-24", health: "Medium", hallucinationRate: 10 },
            { date: "2025-10-25", health: "Low", hallucinationRate: 10 },
            { date: "2025-12-25", health: "Low", hallucinationRate: 10 },
          ],
        },
        {
          agent: "PS Sales Agent",
          testRuns: [
            { date: "2025-11-20", health: "High", hallucinationRate: 20 },
            { date: "2025-11-21", health: "Moderate", hallucinationRate: 20 },
            { date: "2025-11-22", health: "Medium", hallucinationRate: 20 },
            { date: "2025-10-23", health: "Low", hallucinationRate: 20 },
            { date: "2025-12-25", health: "Moderate", hallucinationRate: 20 },

          ],
        }, {
          agent: "resume Parser",
          testRuns: [
            { date: "2025-12-20", health: "Moderate", hallucinationRate: 10 },
            { date: "2025-12-23", health: "Low", hallucinationRate: 50 },
            { date: "2025-12-24", health: "Medium", hallucinationRate: 10 },
            { date: "2025-10-25", health: "Low", hallucinationRate: 10 },
            { date: "2025-12-25", health: "Low", hallucinationRate: 10 },
          ],
...
export class ConversationService {
  ...
  async getAgentHealthData(agenstsIds: string[]) {
    try {
        ...
        agents.map(async (agent:any) => {
          ...
          const testRuns = await Promise.all(
            runs.map(async (run:any) => {
              const testConversations = await prisma.test_conversations.findMany({
                where: { run_id: run.id },
                select: { id: true },
              });

              const total = testConversations.length;
              ...
              for (const tc of testConversations) {
                ...
                if (messages.some((msg:any) => msg.metrics?.isHallucination == true)) {
                  hallucinated++;
                }
              }
              ...
              if (hallucinationRate > 75)  hallucination = 'High';
              else if (hallucinationRate >= 50) hallucination = 'Moderate';
                else if (hallucinationRate >= 25) hallucination = 'Medium';
              else hallucination = 'Low';
              
              return {
                date: run.created_at?.toISOString().slice(0, 10) || '',
                health: run.agent_health, 
                hallucinationRate,
                id: run.id,
