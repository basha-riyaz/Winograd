...
export class AgentConfigService {
  async getAllAgentConfigs(): Promise<any[]> {
    try {
      ...
      return configs.map((config) => ({
        ...
        statuscheck: config.agent_status_at,
        agenthealth: config.agent_health,
        lastrun: config.last_test_run,
        endpoint: config.endpoint,
        inputdata: config.input_format,
        headers: config.agent_headers.reduce(
          (acc, header) => ({
            ...acc,
    ...
    return await prisma.$transaction(async (tx) => {
      try {
        if (config.id) {
          ...
          await tx.agent_descriptions.deleteMany({
            where: { agent_id: config.id },
          });
          await tx.agent_user_descriptions.deleteMany({
            where: { agent_id: config.id },
          });
          await tx.validation_rules.deleteMany({
            where: { agent_id: config.id },
          });
      ...
      scenarioIds.forEach(async (id) => {
        ...
        select_run_ids
          .then(async (conversations) => {
            ...
            console.log(
              `Deleting test runs for scenario ID: ${id} with run IDs: ${runIds}`
            );
          ...
          .catch((err) => {
            console.error(
              `Error deleting conversations for scenario ID ${id}:`,
              err
            );
          });
      });
  ...
  //manasa-s
  async getLatestTestRun(agentId: string) {
    return await prisma.test_runs.findFirst({
  ...
  async updateAgentStatus(agentId: string,status: string, health: string): Promise<void> {
    try {
      if (status !== '') {
        await prisma.agent_configs.update({
          ...
          data: {
            agent_status: status,
            last_run: new Date(), // <-- update last_run column
          },
        });
      } else if (health !== '') {
        await prisma.agent_configs.update({
      ...
      console.log("agent config update error :", error);
    }
  } 
  //manasa-e

  async updateAgentHealth(id: string,agentHealth: string, agentID: string): Promise<void> {
    try {
      console.log('Attempting to update agent status:', { id, agentHealth, agentID });
      if (agentHealth && agentHealth !== '') {
        ...
        console.log('Agent status and last_run updated in DB');
      } 
    } catch (error) {
      console.log("agent config update error :", error);
    }
