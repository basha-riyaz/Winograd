...
export class TestRunService {
  ...
  //  added , to receive the unique agent details
  async getUniqueTestRun(agentname: string): Promise<TestRun[]> {
    try {
      let isHellucinate = false;
      //!to find the last 5 test runs for the agent   
      const testRuns = await prisma.$queryRaw<{ id: string }[]>`SELECT 
                                TO_CHAR(created_at, 'MM-DD-YYYY') AS date,
                                CAST(SUM(failed_tests) AS INTEGER) AS failed,
                                CAST(SUM(passed_tests) AS INTEGER) AS passed
                              FROM 
                                test_runs
                              WHERE 
                                name = ${agentname}
      ...
      //! to find the latest test run for the agent 
      const testRunRows = await prisma.$queryRaw<{ id: string }[]>`SELECT DISTINCT ON (name) id FROM test_runs
        WHERE name = ${agentname} ORDER BY name, created_at DESC;`;
      const testRun = testRunRows.map(row => row.id);
        ...
        testrunhistory: runs.map((run) => ({
          ...
          chats: run.test_conversations.map((tc) => {
            ...
            return {
              ...
              responseTime: conversation.conversation_messages
                .filter((msg) => msg.role === "assistant")
                .map((msg) => msg.response_time ?? 0)[0] || '---',
              hallucination: conversation ? conversation.conversation_messages.some((msg) => msg.role === 'assistant' && msg.metrics?.isHallucination === true) : isHellucinate,
              // timestamp: conversation.created_at
